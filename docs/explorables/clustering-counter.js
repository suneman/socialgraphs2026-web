/* The clustering coefficient, made clickable. Node A sits in the middle;
   six potential neighbours ring it. Attach or detach a neighbour and the
   denominator k(k−1)/2 changes; link two neighbours and the numerator e_A
...
(function () {
  ...
  function neighbours() { return attached.map((on, i) => (on ? i + 1 : -1)).filter((i) => i > 0); }
  ...
  function draw() {
    ...
    // chords among attached neighbours: solid if linked, dashed ghost if not; both clickable
    ...
    $("formula").innerHTML = k < 2
      ? `C<sub>A</sub> is undefined — A has ${k} neighbour${k === 1 ? "" : "s"}, so there are no pairs to check (we set it to 0).`
  ...
  $("random").addEventListener("click", () => {
    ...
    if (neighbours().length < 2) attached[0] = attached[1] = true;
    links = new Set();
    const nb = neighbours();
